...
const EditarCadastro = () => {
    ...
    useEffect(() => {
        const fetchUsuario = async () => {
            try {
                const res = await fetch(`http://localhost:3334/usuarios/${id}`);
    ...
    const onSubmit = handleSubmit(async (data: Usuario) => {
        try {
            ...
            const checkDuplicado = await fetch(`http://localhost:3334/usuarios?email=${data.email}`);
            const usuariosDuplicados: Usuario[] = await checkDuplicado.json();

            if (usuariosDuplicados.some((u) => u.id !== id)) {
